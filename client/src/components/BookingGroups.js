...
function BookingForm({ props }) {
  ...
  const getGuestsCategories = () => {
    if (props.occupancy <= 15) {
      return [`1-${props.occupancy}`];
    } else {
      // Divide occupany in groups of 15 passenger until capacity is reached
      const groups = [];
      let group = 1;
      let passengers = 15;
      while (passengers <= props.occupancy) {
        groups.push(`${group}-${passengers}`);
        group += 15;
        passengers += 15;
        if (passengers > props.occupancy) {
          groups.push(`${passengers - 15}-${props.occupancy}`);
        }
      }
      return groups;
    }
